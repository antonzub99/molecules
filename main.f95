...
!cс
!cс  E  - energy in Ry,
!cс  L  - orbital momentum
!cс  Z > 0 for attraction - charge
!cс -------------------------
...
!      do kn=1,78
!         write(111,211) kn, points(kn), potent(kn), heskwf(kn)
!      end do
...
!c** open(unit=225,file='interpolated_potential.txt',status='replace')
!c** interpolating screen potential and finding dwba potetial in rp(i)
      do k=1,nrp
        xx=rp(k)
        if(xx.lt.points(1)) then
!c temporary option: linear interpolation of screening potential
           ww = 1.d0 -(1.d0 - potent(1))*rp(k)/points(1)
           goto 101
        end if
        if (rp(k).gt.points(78)) then
           ww = 1.d0/Z
           goto 101
        end if
        call parind(xx,points,potent,78,ww)
!c        call pl10(points,potent,xx,20,eps,ww,ker)
101     continue
        scr(k)=Z*ww
        write(225,*)rp(k)," ",scr(k)
!c**        if (rp(k).gt.points(78)) scr(k)=asimpt
!c**        !write(225,*)rp(k),' ',scr(k)
          vd(k)=Z-scr(k)
!        write(111,211) k, xx, ww, scr(k)
211     format(I6, 5d13.6)
      end do

      open(unit=229,file="interpolated_carbon_hesk.txt",status="replace")
! interpolating wavefunction and finding it in rp(i)
      do k=1,nrp
        xx=rp(k)
        if(xx.lt.points(1)) then
! temporary option: quadratic interpolation of heskwf near the origin
           aan = points(2)*heskwf(1)-points(1)*heskwf(2)
           bbn = points(1)**2*heskwf(2)-points(2)**2*heskwf(1)
           zzn = points(1)*points(2)*(points(1)-points(2))
           ww = aan/zzn*xx**2 + bbn/zzn*xx
           goto 102
        end if
! putting zero for the tail of the wave function
        if(xx.gt.points(78)) then
           ww = 0.d0
           goto 102
        end if
        call parind(xx,heskme,heskwf,78,ww)
!c       call pl10(heskme,heskwf,xx,20,eps,ww,ker)
102     continue
        ox(k)=ww*dsqrt(rp(k)/(1.141783d0 * rp(k) + 0.5d0))
        write(229,*)rp(k)," ",ox(k)
      end do
      ...
      call dwave(0,E,delpha)
!c**  deltaphase for later calculations
...
!c** open(129,file="mesh.txt",status="replace")
!c** open(99,file="regular_carbon_wf.txt",status="replace")
!c** open(66,file="irregular_carbon_wf.txt",status="replace")
    !do k=1, nrp
      !write(99,*)rp(k),' ',dw(k)
        !write(66,*)arregw(k)
   !end do
...
!        write(229,*)rp(k),' ',ox(k),' ',dip(k)
!         write(111,211) k, rp(k), ox(k), dw(k), dip(k)
      end do

       dmatr = DF67(RP, dip)

    !open (, file="angular_distribution.txt",status="replace")
    !psipr = (0,0)
    !imaginary_one = (0,1)
    !scatter_fact = (cexp(2*imaginary_one*real(deltaphase)) - 1)/(2*imaginary_one*real(SE))
    ...
    !hi_plus = imaginary_one * real(dwmol) - real(arregwmol)
    !anewwf = interpolated dwave regular wfunction

      open(001, file='pad_0.txt',status='replace')
      open(301, file='pad_30.txt',status='replace')
      open(451, file='pad_45.txt',status='replace')
      open(601, file='pad_60.txt',status='replace')
      open(901, file='pad_90.txt',status='replace')

      call angular_distrib(H00, E, PHASE,  0, dmatr, delpha)
      call angular_distrib(H30, E, PHASE, 30, dmatr, delpha)
      call angular_distrib(H45, E, PHASE, 45, dmatr, delpha)
      call angular_distrib(H60, E, PHASE, 60, dmatr, delpha)
      call angular_distrib(H90, E, PHASE, 90, dmatr, delpha)
...
!     do theta =0, 360
!        psipr = (0,0)
!        tmptheta = theta * 2 * PI / 360.d0
!        do k=0, 10
!            call dwave(k, E, PHASE1)
!            call pl10(rp,dw,amolecular_coordinate,20,eps,anewwf,ker)
!            psipr =(((imaginary_one)**k)*(cexp(imaginary_one*real(PHASE1)))*anewwf*Lezh(k,cos(PI-tmptheta)))+psipr
!            write(212, *)k,' ',((imaginary_one)**k),'  ',cexp(imaginary_one*real(PHASE1)),'  ',anewwf,'  ', psipr
!        end do
!        psipr = psipr * 4 * PI
!        ASSUMING RHO - MOLECULAR VECTOR IS FIXED
!        mu = angle(e, rho) - free parameter
!        theta = angle(rho, k) - graphics is function of theta
!        mu + theta = angle (e, k)
!        mu = 90.d0 * 2 * PI / 360.d0
!        scal1 = cos(dble(mu+tmptheta))
!        scal2 = cos(dble(mu))
!        angular_factor=scal1+imaginary_one*cexp(-imaginary_one*real(PHASE))*scatter_fact*real(E)*hi_plus*psipr*scal2
!        H_big(theta) = (dmatrixel**2) * (cabs(angular_factor))**2
!        write(1488,*) theta, '  ',H_big(theta)
!     end do
       end


      subroutine angular_distrib(hd,E,PHASE,mu,dmatr,delpha)
      ...
      real(8) Lezh
      complex(8) ci,c2i,czero,cone,ctwo,angfac,sctfac,hiplus,psipr
      ...
      sctfac = (cdexp(c2i*dcmplx(delpha,0.d0)) - cone)/(c2i*dcmplx(SE,0.d0))
      amocrd = 2.324364d0

!c**pl10 - subroutine for interpolation

!c      call pl10(rp,dw,amocrd,20,eps,dwmol,ker)
!c      call pl10(rp,arregw,amocrd,20,eps,aregmo,ker)
      call parind(amocrd,rp,dw,nrp,dwmol)
      call parind(amocrd,rp,arregw,nrp,aregmo)
      hiplus = ci * dcmplx(dwmol, 0.d0) - dcmplx(aregmo,0.d0)

!c**anewwf = interpolated dwave regular wavefunction

      do k = 0,10
            call dwave(k, E, phaseo)
            call parind(amocrd,rp,dw,nrp,anewwf)
!            call pl10(rp,dw,amocrd,20,eps,anewwf,ker)
            anwf(k) = anewwf
            faza(k) = phaseo
            write(*,*)anewwf, ' ', phaseo
      end do
      do itheta = 0, 360
        psipr = czero
        tmpth = dble(itheta) * 2.d0 * PI / 360.d0
        do k=0, 10
            alezh = (-1.d0)**k*Lezh(k,dcos(tmpth))
            psipr =(ci**k)*cdexp(ci*dcmplx(faza(k),0.d0))*dcmplx(anwf(k)*alezh,0.d0)+psipr
        end do
        psipr = psipr * dcmplx(4.d0 * PI,0.d0)
!c* ASSUMING RHO - MOLECULAR VECTOR - IS FIXED
        !amu = angle(e, rho) - free parameter
        !theta = angle(rho, k) - angular distribution as function of theta
        !amu + theta = angle (e, k)
        amu1 = dble(mu) * 2.d0 * PI / 360.d0
        scal1 = dcos(amu1+tmpth)
        scal2 = dcos(amu1)
        angfac = scal1 + ci*cdexp(-ci*dcmplx(PHASE,0.d0))*sctfac*dcmplx(E*scal2)*hiplus*psipr
        hd(itheta) = dmatr**2 * (cdabs(angfac))**2
!        write(111,555) itheta, hd(itheta), dmatr, angfac
555     format(i6,4d13.5)
      end do
      return
      end
...
!      implicit double precision (a-h, o-z)
!!      real*8 X, Y, XX, Eps, YY, E, E0
!      dimension X(20000), Y(20000)
!      integer M, Error
...
!c** Интегрирование функции F, заданнной таблично, по формуле Симпсона
...
!c*** Интегрирование функции F, заданнной таблично
!c*** на неравномерной сетке по формуле трапеций
      implicit double precision (a-h, o-z)
!      double precision, intent(in):: X(:), F(:)
!      integer i, n
      dimension X(20000),F(20000)
!c**begin
      n=size(X)
      S=0.0D0
      do i=1, n-1
      ...
      double precision function Lezh(n,x)
      integer n
